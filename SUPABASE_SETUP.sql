...
-- Admin-only full account delete (auth.users → cascades public.users). No service_role key in app.
...
-- ⚙️ App settings (admin toggles — registration OTP, etc.)
...
-- Registration email OTP (pre-signup verification)
...
-- Booking orders + consultation payments
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    CREATE POLICY "Professionals read confirmed booking orders" ON public.booking_orders
      FOR SELECT TO authenticated
      USING (professional_id = auth.uid() AND status = 'confirmed');
  END IF;
...
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  SELECT * INTO v_row FROM public.booking_orders WHERE id = p_order_id FOR UPDATE;
  IF v_row.id IS NULL OR v_row.user_id IS DISTINCT FROM v_uid THEN RAISE EXCEPTION 'Forbidden'; END IF;
  IF v_row.status IS DISTINCT FROM 'processing' THEN RETURN FALSE; END IF;
  IF v_row.guest_appointment_id IS DISTINCT FROM p_guest_appointment_id THEN RAISE EXCEPTION 'Appointment mismatch'; END IF;
  IF v_row.amount_paise > 0 THEN
    INSERT INTO public.payments (
      booking_order_id, guest_appointment_id, client_id, professional_id,
      amount, status, payment_method, transaction_id
    ) VALUES (
      v_row.id, p_guest_appointment_id, v_row.user_id, v_row.professional_id,
      v_row.amount_paise, 'completed', COALESCE(p_payment_method, 'mock'), p_transaction_id
    );
  END IF;
  UPDATE public.booking_orders SET status = 'confirmed', confirmed_at = NOW(), updated_at = NOW() WHERE id = p_order_id;
  RETURN TRUE;
END;
$$;
